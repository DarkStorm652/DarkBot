...
public class CLISpamBotWrapper extends MinecraftBotWrapper {
	...
	private CLISpamBotWrapper(MinecraftBotData data, String owner) throws AuthenticationException, UnsupportedProtocolException {
		super(data);
		synchronized(bots) {
			bots.add(this);
		}
					...
					if(!offline) {
						...
						user: while(true) {
							...
							while(true) {
								...
								try {
									CLISpamBotWrapper bot = new CLISpamBotWrapper(generateData(server, session.getUsername(), session.getPassword(), authService, session, protocol, null, proxy), owner);
						...
						while(true) {
							...
							try {
								...
								CLISpamBotWrapper bot = new CLISpamBotWrapper(generateData(server, username, null, null, null, protocol, null, proxy), owner);
								while(bot.getBot().isConnected()) {
									try {
										Thread.sleep(500);
									} catch(InterruptedException exception) {
