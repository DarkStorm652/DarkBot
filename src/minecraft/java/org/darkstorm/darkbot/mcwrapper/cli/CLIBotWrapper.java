...
public class CLIBotWrapper extends MinecraftBotWrapper {
	private CLIBotWrapper(MinecraftBotData data, String owner) throws AuthenticationException, UnsupportedProtocolException {
		super(data);
		addOwner(owner);
		...
		if(!offline) {
			...
			user: do {
				...
				while(true) {
					...
					try {
						CLIBotWrapper bot = new CLIBotWrapper(generateData(server, session.getUsername(), session.getPassword(), authService, session, protocol, null, proxy), owner);
			...
			while(true) {
				...
				try {
					...
					if(username == null)
						...
						name = username;
					CLIBotWrapper bot = new CLIBotWrapper(generateData(server, name, null, null, null, protocol, null, proxy), owner);
					while(bot.getBot().isConnected()) {
						try {
							Thread.sleep(1500);
						} catch(InterruptedException exception) {
